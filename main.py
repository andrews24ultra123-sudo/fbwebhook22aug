...
# - Mint/Burn alert: asset in MINT_ASSET_IDS → Internal Wallet (name "N/A")
# - Withdrawal alert: asset in QCDT_TOKEN_IDS → One Time Address (name "N/A")
# - Filters:
#     * Any event with src/dst name in FILTER_NAME_BLOCKLIST
#     * Any investor in INVESTOR_BLOCKLIST (case-insensitive match)
...
FILTER_NAME_BLOCKLIST   = {"op@fundadmin.com"}       # suppress by src/dst name
INVESTOR_BLOCKLIST      = {"zhuyaowu@tandidi.com"}   # suppress by investor email
...
def should_suppress(norm: dict) -> bool:
    # filter by src/dst name
    for bad in FILTER_NAME_BLOCKLIST:
        if bad.lower() in norm.get("src_name","").lower() or bad.lower() in norm.get("dst_name","").lower():
            return True
    # filter by investor
    investor = norm.get("src_name") or norm.get("dst_name") or ""
    for bad in INVESTOR_BLOCKLIST:
        if investor.lower() == bad.lower():
            return True
    return False
...
# ─────────────────────────── Routes ───────────────────────────
@app.get("/") async def root(): return {"ok":True,"webhook":"/fireblocks/webhook"}
...
async def fireblocks_webhook(
    ...
    if should_suppress(norm):
        logging.info(f"Suppressed: investor or name blocklist hit tx={norm.get('tx_id')}")
    ...
    logging.info(f"Suppressed: no rule match asset={asset_upper} tx={tx_id}")
